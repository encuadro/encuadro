       ...
       // printf("xx-xc: %f\n",xx-xc);
        ...
        for(y=0;y<aux->ysize;y++)
        {
            ...
            for(i=0;i<kernel->dim;i++)
            {
                ...
                while( j < 0 ) j += float_x_size; /*Aca supone a la imagen periodica*/
...
//static image_float gaussian_sampler( float* in, int width,int height, int d, float scale,
//float sigma_scale )
image_float gaussian_sampler2( image_float in, float scale, float sigma_scale )
{
    image_float aux,out;
    ntuple_list kernel;
    unsigned int N,M,h,n,x,y,i;
...
//static image_float gaussian_sampler( float* in, int width,int height, int d, float scale,
//float sigma_scale )
image_float gaussian_sampler3( image_float in, float scale, float sigma_scale )
{
    image_float aux,out;
    ntuple_list kernel;
    unsigned int N,M,h,n,x,y,i;
    ...
    for(x=3;x<aux->xsize-3;x++)
    {
        ...
        for(y=0;y<aux->ysize;y++)
        {
            ...
            for(i=0;i<kernel->dim;i++)
            {
                ...
                /* symmetry boundary condition */
//                while( j < 0 ) { j += float_x_size; printf("print changamos 1\n");}
//                    while( j >= float_x_size ) { j -= float_x_size; printf("print changamos 2\n");}
//                if( j >= (int) in->xsize ) {j = float_x_size-1-j; printf("print changamos 3\n");}
