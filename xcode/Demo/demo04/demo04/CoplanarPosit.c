...
//    printf("%f\t %f\t %f\n",Trans[0],Trans[1],Trans[2]);
    ...
    {
        Error(NbPts,centeredImage,worldPts,focalLength,center,RotFinal1,TransFinal1,&E1,&Ep1,&Ehvmax1);
        Error(NbPts,centeredImage,worldPts,focalLength,center,RotFinal2,TransFinal2,&E2,&Ep2,&Ehvmax2);
...
//    printf("%f\t %f\t %f\n",Trans[0],Trans[1],Trans[2]);
    ...
    if ((RotFinal1[0][0]!=2)&&(RotFinal2[0][0]!=2))
    {
        Error(NbPts,centeredImage,homogeneousWorldPts,focalLength,center,RotFinal1,TransFinal1,&E1,&Ep1,&Ehvmax1);
        Error(NbPts,centeredImage,homogeneousWorldPts,focalLength,center,RotFinal2,TransFinal2,&E2,&Ep2,&Ehvmax2);
...
//    printf("\n");
    ...
    if (zmin1<0) Rot1[0][0]=2;
    if (zmin2<0) Rot2[0][0]=2;
...
void  PerspMoveAndProj(int N, double **obj, double** r, double* t, double foc, double** proj) /*Image projection given a rotation and a traslation.*/
{
    double  **moved;
    long int    i,j,k;
...
void Error(long int NP,double** impts,double** obpts,double f,double center[2],double** Rotat,double* Translat,double* Er,long int* Epr,double* Erhvmax)
...

{
    ...
    {
        PerspMoveAndProj(NP,obpts,Rotat,Translat,f,impredic); /*Project the image with the current pose*/
...
void PositLoop(int NbPts, double **centeredImage, double** homogeneousWorldPts, double**objectMat, double f,double center[2], double** RotIn, double* TransIn,double** Rot, double* Trans){
    ...
    while (!converged) {
        ...
        if (count!=0) {
            ...
            Error(NbPts, centeredImage, homogeneousWorldPts, f,center, Rot1, Trans, &Er1, &Epr1, &Erhvmax1);//tiene que entrar los puntos en la imagen no centrados.
            Error(NbPts, centeredImage, homogeneousWorldPts, f,center, Rot2, Trans, &Er2, &Epr2, &Erhvmax2);
