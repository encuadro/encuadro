...
int getIncompleteQlSetArr(int qlListSize, quadrilateral *qlList,
	...
	if (markerIsComplete == 1) {
		if (VERBOSE) printf("MSG: MRKR_COMPLETE_FOUND\n");
		return MRKR_COMPLETE_FOUND;
	} else if (markerIsSufficient == 0) {
		if (VERBOSE) printf("MSG: MRKR_NOT_SUFFICIENT\n");
	...
	if (not_found==1){
		if (VERBOSE) printf("MSG: MRKR_INCOMPLETE_NOT_FOUND\n");
		return MRKR_INCOMPLETE_NOT_FOUND;
	}

	if (VERBOSE) printf("MSG: MRKR_INCOMPLETE_FOUND\n");
...
int orderQlSetArr3(quadrilateralSet *qlSet) {
	...
	float c[3];
	float a[3] = {vect[ref_index][0],vect[ref_index][1],0};
	float b[3] = {vect[(ref_index+1)%MRKR_NB_QLSETS][0],vect[(ref_index+1)%MRKR_NB_QLSETS][1],0};
...
int findPointCorrespondances(int *listSize, float *list, float **imgPts) {
	...
	if (*listSize >= 28) {
		...
		if (VERBOSE) error_code = MRKR_NOT_ENOUGH_SEGMENTS;
