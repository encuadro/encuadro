...
// hay que hace runa funcion igual a esta pero que levante de la base de datos en vez de disco
int* levantarDescriptor(char *id_obra, int* nKeyPoints){ //levanta el descriptor desc en la base de datos
	...
	char *database = "proyecto"; //nombre de la base de datos a consultar
	...
	char sql[15100];
	strcpy(sql,"select id_descriptor,descriptor from proyecto.descriptor where id_obra ='");
	strcat(sql,id_obra);
...
void escribirDescriptorPrueba(char* nombre, int* descriptor, long int largo)
{
    FILE *archivo;   
...
/*
	while ((row = mysql_fetch_row(res)) != NULL) // recorrer la variable res con todos los registros obtenidos para su uso
	printf("%s\t%s\t%s \n", row[0],row[1], "algo"); // la variable row se convierte en un arreglo por el numero de campos que hay en la tabla
...
void actualizarBD(char * res, char *id_desc){
	MYSQL *conn; //variable de conexion para mysql
	MYSQL_RES *res2; // variable que contendra el resultado de la consuta
	MYSQL_ROW row; //variable que contendra los campos por cada registro consultado
	char *server = "localhost"; //direccion del servidor 127.0.0.1, localhost o direccion ip
	char *user = "root"; //usuario para consultar la base de datos
	char *password = "root"; // contraseña para el usuario en cuestion
	...
	//Connect to database 
	if (!mysql_real_connect(conn, server, user, password, database, 0, NULL, 0)) { // definir los arametros de la conexion antes establecidos
	fprintf(stderr, "%s\n", mysql_error(conn)); // si hay un error definir cual fue dicho error
	//exit(1);
	}
	...
	char * sql = aux2;
        char aux3[100]; 
	strcpy(aux3,"update proyecto.descriptor set id_obra ='");
	sql = strcat(aux3,res);
	sql =  strcat(sql,"' where id_descriptor='");
        //sql =  strcat(sql,argv[3]);      
	sql =  strcat(sql,id_desc); 
	sql =  strcat(sql,"'");     
	//printf("sql %s\n",sql);
...
int r=0;
    ...
    {
	int *descr; 
	descr = levantarDescBD("91",nKeyPoints);
	printf("entro al 2 levantar dio %ld", descr);
//	 levantarDescBD("91",nKeyPoints);
    }
    if (argc==3)
    {
	...
	nombsala=argv[2];
        ...
        //printf("\nimagen dio %s",image_out);
	
	strcpy(pepe,image_out);
	 //printf("\nconcha dio %s",concha);
	sscanf(pepe,"%d",&r);
	 printf("%d",r);
    }
...
/*
	...
	actualizarBD(res,argv[3]);*/
	
	//actualizarBD(image_out,argv[3]);
