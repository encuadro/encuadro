...
\item \textbf{Tutorial 5:} se ven algunas nuevas primitivas y se muestra cómo agregar transparencia a los objetos.
\end{itemize}
...
\normalsize
En la primera línea de código se instancia la clase \textit{Isgl3dPODImporter} que sirve para transformar modelos POD a objetos ISGL3D, y se le asigna a la misma el modelo ``modelo.pod''. Luego, se crea un nodo llamado ``$\_$model'', al que se le asignará el modelo; y se agrega a la escena. Finalmente, se le asigna al nodo una posición. En la figura \ref{fig:render4} se puede ver un modelo de José Artigas, agregado dos veces a una misma escena, pero visto desde ángulos distintos.\\
