...
De esta manera, se crea una imagen auxiliar vacía y escalada en $x$ pero no en $y$, y se recorre asignándole a cada píxel en $x$ su valor correspondiente, obtenido del promedio del píxel $\frac{x}{escala}$ en la imagen original y sus vecinos, todos ponderados por el filtro Gaussiano. Luego se crea otra imagen, pero esta vez escalada tanto en $x$ como en $y$, y se recorre asignándole a cada píxel en $y$ su valor correspondiente, obtenido del promedio del píxel $\frac{y}{escala}$ en la imagen auxiliar y sus vecinos, todos ponderados por el filtro Gaussiano. En la imagen \ref{fig: lsd_5} se muestra la relación entre las imágenes.\\

\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{figs_lsd/lsd_5.eps}
\caption{Relación entre las imágenes en cuestión. Escala: $0,5$.}
\label{fig: lsd_5}
\end{figure}

Cuando se trabaja con escalas genéricas ...
\subsection{\textit{Level-line angles}}

\subsection{Refinamiento de los candidatos}

 \subsection{Algorirmo en presición simple}
