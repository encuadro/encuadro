...
Sin embargo, LSD plantea un método para ahorraste este tipo de problemas. Durante el proceso de crecimiento de las regiones, también se realiza la aproximación rectangular a dicha región (paso (2) de los tres definidos anteriormente); y si menos cierto porcentaje umbral de los píxeles  dentro del rectángulo corresponden a la \textit{line-support region}, lo que se tiene no es un segmento. Se detiene entonces el crecimiento de la región.
...
El filtrado de la imgen se hace en dos pasos, primero a lo ancho y luego a lo largo. Se utiliza el núceo Gaussiano normalizado de la figura \ref{fig: lsd_4}.\\
...
Originalmente, LSD fue implementado en precisión doble o \textit{double} (64 bits por valor). Sin embargo, el \textit{ipad 2} (dispositivo para el cual se optimizó el algoritmo), cuenta con un procesador \textit{ ARM Cortex-A9}, cuyo bus de datos es de 32 bits. Se decidió entonces probar cambiar al algoritmo a precisión simple o \textit{float} (32 bits por valor) y los resultados fueron realmente buenos. No sólo el algoritmo bajó su tiempo de ejecución, sino que además no existen cambios notorios en el desempe\~no del mismo.
...
\label{fig: lsd_8}
\end{figure}
Se analizaron los tiempos promedio para la ejecución del filtro Gaussiano original y del optimizado, ambos con precisión doble y simple. Las imagen de prueba fue la de la figura \ref{fig: lsd_8}; sépase que por cómo es el algoritmo, el contenido de la imagen en independiente del tiempo de procesamiento en cualquiera de los casos. Los valores relevantes del experimento se muestran en las tablas \ref{tab:gaussian_double} y \ref{tab:gaussian_float}:
\begin{itemize}
...
\end{tabular} 
\caption{Comparación entre los tiempos de ejecución del filtro Gaussiano optimizado y el original. Ambos comprecisión doble.}
...
\end{tabular} 
\caption{Comparación entre los tiempos de ejecución del filtro Gaussiano optimizado y el original. Ambos comprecisión simple.}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.4]{figs_lsd/lsd_9.eps}
...
\begin{tabular}{|c|c|c|} \hline
 ...
 Imagen urilizada								& marcador sint\'etico	& marcador sint\'etico \\ \hline
 Tama\~no de imagen	de entrada	& $480\times 360$		&	$480\times 360$	\\ \hline
 Escala												&	$0,5$						&	 	$0,5$				\\ \hline
 Tama\~no de imagen de salida		& $240 \times 180$	& $240 \times 180$ \\ \hline
  Segmentos detectados					& $36$						& $36$ \\ \hline
...
\end{tabular} 
\caption{Comparación entre los tiempos de ejecución del filtro Gaussiano más LSD optimizados y los originales, para la imagen \ref{fig: lsd_8}. En todos los casos con comprecisión doble.}
...
\begin{tabular}{|c|c|c|} \hline
 														& Algoritmo original	&	Algoritmo optimizado \\ \hline
Imagen urilizada								& \textit{zebras.png}	& \textit{zebras.png} \\ \hline
Tama\~no de imagen	de entrada	& $480\times 360$		&	$480\times 360$	\\ \hline
Escala												&	$0,5$						&	 	$0,5$				\\ \hline
Tama\~no de imagen de salida		& $240 \times 180$	& $240 \times 180$ \\ \hline
...
\end{tabular} 
\caption{Comparación entre los tiempos de ejecución del filtro Gaussiano más LSD optimizados y los originales, para la imagen \ref{fig: lsd_9}. En todos los casos con comprecisión doble.}
...
\end{tabular} 
\caption{Comparación entre los tiempos de ejecución del filtro Gaussiano más LSD optimizados y los originales, para la imagen \ref{fig: lsd_8}. En todos los casos con comprecisión simple.}
...
\end{tabular} 
\caption{Comparación entre los tiempos de ejecución del filtro Gaussiano más LSD optimizados y los originales, para la imagen \ref{fig: lsd_9}. En todos los casos con comprecisión simple.}
