...
A diferencia de otros sistemas operativos, Mac OS X por defecto viene con Apache y Php instalados. Para tener un servidor entonces, basta con activarlos. En caso de querer contar con MySQL, es necesario entonces instalarlo. La activación del servidor es un proceso muy simple del cual existe bastante material disponible en internet.
...
En el Capítulo \ref{chap: casoUso} se buscó resolver los aspectos técnicos de las distintas ideas interesantes a ser aplicadas en un prototipo de aplicación comercial real. En dicho capítulo se centró la atención en resolver simplemente a nivel técnico, dejando de lado los aspéctos estéticos y artísticos. Así entonces la idea de generar un \textit{render} de un perro resuelve el mismo problema (sin demasiado valor para un museo quizá) que la idea de generar un \textit{render} con el modelo de José Artigas. Lo mismo sucede con el caso de uso de proyectar un video sobre el marcador cuando se lo compara con la proyeccción del video sobre parte de la obra. En el presente capítulo se buscó llevar dentro de lo posible, los casos de uso mencionados, a una implementación que pueda llegar a resultar atractiva para los responsables de un museo. Es posible que existan otras ideas no presentadas en el presente capítulo que utilicen las herramientas técnicas resueltas en el Capítulo \ref{chap: casoUso} y que tengan aún un valor artístico superior a lo presentado hasta el momento, lo que deja una puerta abierta en este sentido a seguir explorando ideas innovadoras que resulten atractivas para museos.
