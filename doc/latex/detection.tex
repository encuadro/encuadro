...
\caption{Detalle de un QlSet. A la izquierda se muestra el resultado de la detección de un $QlSet$ y el orden interno de sus \emph{cuadriláteros} y a la derecha el orden de los vértices respecto al sistema de coordenadas local.}
\label{fig:QlSetDetail}
\end{figure}

Un detalle del marcador completo se muestra en la figura \ref{fig:MarkerDetail} en donde se define el conjunto $i$ de cuadriláteros concéntricos como el $QlSet[i]$ y se definen los respectivos centros de cada uno de ellos como $\mathbf{c}_i$. El sistema de coordenadas del \emph{marcador QR} tiene centro en el centro del $QlSet[0]$ y ejes de coordenadas idénticos al definido para cada $Ql$. Se tiene además que los ejes de coordenadas pueden ser obtenidos mediante los vectores normalizados,
\begin{equation}
\begin{split}
...
\subsubsection{Filtrado y agrupamiento de segmentos}
...
\begin{figure}[h!]
  ...
  \subfigure[Entrada: segmentos de línea detectados por LSD]{
    \includegraphics[scale=0.45]{figs_detection/lsd.png}}
  \subfigure[Salids: segmentos de línea filtrados y agrupados]{
...
\begin{figure}[h!]
  ...
  \caption{Resultados del algoritmo de determinación de correspondencias robustificado para una falla en el filtrado de segmentos.}
\end{figure}
