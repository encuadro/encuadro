...
Como se vió algunos párrafos atrás, el proceso mediante el cual se calculan los parámetros intrínsecos reales de una cámara es denominado ``calibración de cámara''. Existen varios métodos para calibrar una cámara, entre los que se destacan el método de Zhang \cite{Zhang99:8} y el método de Heikkilä y Silvén \cite{HeikkilaS97}. En este proyecto, para calibrar las cámaras se utilizó una implementación en \textit{Matlab} basada en el método de Zhang (\cite{camCalib12}), que afortunadamente dió resultados muy buenos. Por eso, se explicará a continuación, de forma breve, cómo funciona este método.\\

El método de Zhang es muy sencillo y flexible. Sólo requiere de la cámara a calibrar, una computadora y una imagen patrón (plana), que puede ser impresa en cualquier impresora doméstica y pegada en cualquier superficie razonablemente plana y dura, a la que se le tomarán al menos dos fotografías desde orientaciones distintas. Ni las posiciones de la cámara en cada caso, ni el movimiento entre ellas tienen por qué ser conocidos. Este método devuelve los parámetros intrínsecos de la cámara correspondientes al modelo \textit{pin-hole} visto algunos párrafos antrás, sus parámetros extrínsecos para cada fotografía utilizada para la calibración y la distorsión radial de sus lentes.\\
...
m = I.E.M
\]
donde $E$ representa a la matriz de parámetros extrínsecos e $I$ representa a la matriz de par\'metros intrínsecos de la cámara. Además:
\[
I =
\left( \begin{array}{ccc}
\alpha & s & u'_P\\ 
...
\end{array} \right)
\]
donde $(X_{m0}, Y_{m0}, Z_{m0}, 1 )^T$ denotan las coordenadas homogéneas del punto \textbf{M} respecto de los ejes del mundo y $(u'_0,v'_0,1)^T$ representan las coordenadas homogéneas de su proyección en el plano imagen, \textbf{m}, respecto de los ejes normalizados de la imagen. Se le llamó $r_i$ a la i-ésima columna de la matriz rotación de los parámetros extrínsecos de la cámara.\\

Dada una fotografía de la imagen patrón plana, es posible estimar una homografía que relacione a ambas. Si se toma en cuenta dicha homografía $(PY)$ vale $PY =(h_{1}, h_{2}, h_{3}) = I.(r_{1}, r_{2}, t)$ y que las columnas $r_1$ y $r_2$ son ortonormales, realizando algo de matemática se llega a que:
\[
\begin{array}{c}
...
\right)
\]
se demuestra fácilmente que la misma es simétrica, por lo que quedará absolutamente definida por un vector de 6 dimensiones:
\[
...
\]
donde se referencia a la i-ésima columna de PY como $h_i = (h_{i1}, h_{i2}, h_{i3})^T$; se tiene que:
\[
