...
\end{equation}
Se puede ver que los términos $x_i(1+\epsilon_i)$ y $y_i(1+\epsilon_i)$ son las coordenadas $(x^{'}_i,y^{'}_i)$ de la SOP. 
En la expresion de $\epsilon_i$ en \ref{eq_4}, el producto escalar da la coordenada  \textit{z} de $M_0M_i$, $Z_i - Z_0$, entonces se tiene que 
\begin{equation*}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.45]{figs_posit/posit_6}
\label{fig: posit_6}
...
que es la proyección SOP de la intersección de la línea de vista del punto $m_i$ con el plano $G''$, esto esta demostrado en \cite{David02softposit}. La pose encontrada es correcta cuando ambos lados de \ref{eq_24} son iguales. Por lo tanto la ecuacion que se busca minimizar es la siguiente:
...
\end{split}
\end{equation}
y $M_0M_i$ se toma en coordenadas homogeneas. 
...
\subsection{Calculo de pose sin correspondencias}
...
\begin{itemize}
\item[1] Se calculan las variables de la matriz de asignación asumiendo todo lo demas conocido.
...
Se busca tener una matriz \textit{m} que indique las correspondencias entre los \textit{N} puntos detectados y los \textit{M} puntos en del modelo, y ademas minimize \textit{E}. 
...
\end{equation*}
en donde $\beta$ es una constante muy peque?a y la fila $N+1$ y la columna $M+1$ son inicializadas con constantes peque?as. Luego se itera utilizando los siguientes pasos hasta obtener la matriz \textit{m}.
\begin{itemize}
\item[1] Se normaliza cada fila y columna por la suma de los elementos de esa fila o columna respectivamente hasta que $\Vert m^i - m^{i-1}\Vert$ sea peque?o. La matriz resultante cumple que todas la filas y columnas suman 1.
