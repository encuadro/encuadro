...
Liste ORTJunctionListID;

	SortList ( ORTJunctionListID ,SortByIDVLJunctions);
...
long  SortByIDVLJunctions (a, b)

  long *a, *b;	/* Dummy variables */

{
  struct ORTVJunction *element1;
  struct ORTVJunction *element2;

/*
 First we must cast the dummy variables into ORTJunction structures

*/
  	element1 = MACCast(ORTVJunction, *a);
  	element2 = MACCast(ORTVJunction, *b);
/*
 Now introduce the sorting criterion

*/

  if ( element2->FirstID  < element1->FirstID ||
      (element2->FirstID == element1->FirstID &&
	element2->SecondID < element1->SecondID) ) {
	return(TRUE);
  } else {
  	return(FALSE);
  }
