...
void ReadIPEGTriplets (des, Lst, mark)
  FILE *des;
...
#define MAXLINE 500
  ...
  static char whites[] = " \t\n";
  char *ret;
  C_Triplet *new_pair;		/* Created Pair */
...
*/
  ...
  while ((ret = fgets (textline, MAXLINE, des)) && (ret[0] != NXT)) {
    new_pair = (C_Triplet *) malloc (sizeof (C_Triplet));
    sscanf (ret, "%d %d %d %d", &junki, &(new_pair->x),&(new_pair->y),&(new_pair->z));
